...
description: AWS 스팟 인스턴스 환경에서 발생한 Swap 메모리 관련 에러의 원인 분석과 CloudWatch 로깅을 통한 디버깅 과정을 공유합니다.
...
lang: ko-KR
faq:
  - question: "스팟 인스턴스에서 swap memory 체크가 실패하는 주요 원인은 무엇인가요?"
    answer: "인스턴스 셋업 시 설정한 swap memory가 Java 애플리케이션의 메모리 체크 단계에서 아직 적용되지 않은 경우 발생합니다. ManagementFactory로 조회 시 total, free swap이 0으로 나타나면 세팅이 완료되지 않은 것입니다."
  - question: "연쇄 장애가 발생한 근본 원인은 무엇인가요?"
    answer: "swap memory가 부족하다고 잘못 판단하여 AI 프로세스를 재시작했고, 재시작 실패 시 종료 코드 처리가 불완전했으며, 서버 종료 메서드도 에러를 catch하지 못하는 레거시 문제가 겹쳐 연쇄 장애가 발생했습니다."
  - question: "이 문제를 방지하기 위한 핵심 해결책은 무엇인가요?"
    answer: "swap memory 세팅이 완료되었는지 확인한 후에만 메시지 큐를 소모하도록 변경하고, 프로세스 종료 코드에 대한 포괄적인 예외 처리를 추가하며, basicAck 순서를 조정하여 큐 연결이 끊기는 것을 방지해야 합니다."
---
## 서두
